<template>
  <AppLayout :title="`Edit: ${cohort.name}`" description="Update cohort details">
    <template #header-actions>
      <Link :href="route('cohorts.show', cohort.id)" class="text-sm text-gray-500 hover:text-gray-700 font-medium">
        ← Back to Cohort
      </Link>
    </template>
    ...
    <div v-if="confirmDelete" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
        ...
        <div class="flex justify-end gap-3">
          ...
          <Link :href="route('cohorts.destroy', cohort.id)" method="delete" as="button"
...
const submit = () => form.put(route('cohorts.update', props.cohort.id))
</script>

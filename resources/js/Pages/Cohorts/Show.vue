<template>
  <AppLayout :title="cohort.name" :description="`${cohort.department?.name} · ${cohort.academic_year?.name}`">
    <template #header-actions>
      <div class="flex items-center gap-3">
        <Link :href="route('cohorts.index')" class="text-sm text-gray-500 hover:text-gray-700 font-medium">
          ← Cohorts
        </Link>
        <Link v-if="can('edit-cohorts')" :href="route('cohorts.edit', cohort.id)"
    ...
    <div class="space-y-6">
      ...
      <div class="bg-white rounded-xl border border-gray-200">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          ...
          <Link v-if="can('create-students')" :href="route('users.create')"
        ...
        <div class="divide-y divide-gray-50">
          ...
          <div v-for="student in cohort.students" :key="student.id"
            ...
            <Link :href="route('users.show', student.user_id)"

...
const form = useForm({
    ...
    resources: props.lessonPlan.resources || '',
    assessment: props.lessonPlan.assessment || '',
    lesson_date: props.lessonPlan.lesson_date,
    start_time: props.lessonPlan.start_time || '',
    end_time: props.lessonPlan.end_time || '',
    timetable_slot_id: props.lessonPlan.timetable_slot_id || '',
    status: props.lessonPlan.status,
});

const submit = () => {
    form.patch(route('hive.modules.lesson-plans.update', [props.module, props.lessonPlan]));
};
</script>
...
<template>
    ...
    <HiveLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Lesson Plan</h2>

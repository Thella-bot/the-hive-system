<template>
  <HiveLayout title="Applications" description="Manage student applications">
      ...
      <table class="w-full text-sm">
        <thead class="bg-gray-50 border-b border-gray-200">
          <tr>
            ...
            <th class="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Study Option</th>
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-if="applications.data.length === 0">
            <td colspan="5" class="px-6 py-12 text-center text-gray-400">No applications found.</td>
          </tr>
          <tr v-for="application in applications.data" :key="application.id" class="hover:bg-amber-50 transition-colors">
            ...
            <td class="px-6 py-4">
              <div class="flex items-center justify-end gap-3">
                <Link :href="route('hive.applications.show', application.id)"
    ...
    <Pagination v-if="applications.data.length > 0" :links="applications.links" :meta="applications.meta" />
  </HiveLayout>
</template>

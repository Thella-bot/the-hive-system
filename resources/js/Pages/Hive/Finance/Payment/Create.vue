...
const props = defineProps({
  ...
  selectedInvoiceId: { type: [String, Number], default: null },
});

const form = ref({
  invoice_id: props.selectedInvoiceId ?? '',
  items: [{ description: '', qty: 1, unit_price: '', total: '' }],
  payment_method: 'cash',
  payment_date: '',
  notes: '',
});
...
const submit = () => {
  ...
  router.post(route('hive.finance.payments.store'), form.value, {
...
<template>
  ...
  <HiveLayout title="Create Payment" description="Record a new payment">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
              <tr v-for="(item, index) in form.items" :key="index">
                <td class="px-3 py-2">
                  ...
                  <span v-if="errors[`items.${index}.description`]" class="text-red-500 text-xs">{{ errors[`items.${index}.description`] }}</span>
                </td>
                <td class="px-3 py-2 text-center">
                  ...
                  <span v-if="errors[`items.${index}.qty`]" class="text-red-500 text-xs">{{ errors[`items.${index}.qty`] }}</span>
                </td>
                <td class="px-3 py-2 text-right">
                  <input v-model="item.unit_price" type="number" step="0.01" min="0" required
                    ...
                    @input="updateItemTotal(index)" />
                  <span v-if="errors[`items.${index}.unit_price`]" class="text-red-500 text-xs">{{ errors[`items.${index}.unit_price`] }}</span>
                </td>
                <td class="px-3 py-2 text-right">
                  <input v-model="item.total" type="number" step="0.01" min="0" required
                    class="w-full px-2 py-1 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 text-sm text-right" />
                  <span v-if="errors[`items.${index}.total`]" class="text-red-500 text-xs">{{ errors[`items.${index}.total`] }}</span>
                </td>
                <td class="px-3 py-2 text-center">

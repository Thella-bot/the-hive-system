  ...
  <HiveLayout title="Financial Dashboard" description="Overview of financial performance">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold mt-1" :class="(metrics?.net_position ?? 0) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">
                {{ formatCurrency(metrics?.net_position) }}
              </p>
            </div>
            <div :class="(metrics?.net_position ?? 0) >= 0 ? 'bg-green-100 dark:bg-green-900/30' : 'bg-red-100 dark:bg-red-900/30'" class="p-3 rounded-lg">
              <CurrencyDollarIcon :class="(metrics?.net_position ?? 0) >= 0 ? 'w-6 h-6 text-green-600' : 'w-6 h-6 text-red-600'" />
            </div>
          </div>
      </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold mt-1 text-gray-900 dark:text-gray-100">{{ (metrics?.collection_rate ?? 0) }}%</p>
            </div>
          ...
          <div class="mt-3">
            <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
              <div class="bg-blue-600 h-2 rounded-full transition-all" :style="{ width: `${Math.min(metrics?.collection_rate ?? 0, 100)}%` }"></div>
            </div>
          </div>
      </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-lg font-semibold text-red-600 dark:text-red-400">{{ invoices?.overdue_count ?? 0 }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 mb-6">
      <div class="flex items-center justify-between mb-4">
        ...
        <Link :href="route('hive.finance.budgets.index')" class="text-sm text-amber-600 hover:text-amber-700">View All</Link>
      </div>
    ...
    <div class="mt-6 flex flex-wrap gap-3">
        <Link :href="route('hive.finance.invoices.index')" class="inline-flex items-center gap-2 bg-amber-100 dark:bg-amber-900/30 hover:bg-amber-200 dark:hover:bg-amber-900/50 text-amber-800 dark:text-amber-400 px-4 py-2 rounded-lg text-sm font-medium transition-colors">
        <DocumentTextIcon class="w-4 h-4" />
        Invoices
      </Link>
      <Link :href="route('hive.finance.payments.index')" class="inline-flex items-center gap-2 bg-amber-100 dark:bg-amber-900/30 hover:bg-amber-200 dark:hover:bg-amber-900/50 text-amber-800 dark:text-amber-400 px-4 py-2 rounded-lg text-sm font-medium transition-colors">
        <BanknotesIcon class="w-4 h-4" />
        Payments
      </Link>
      <Link :href="route('hive.finance.expenses.index')" class="inline-flex items-center gap-2 bg-amber-100 dark:bg-amber-900/30 hover:bg-amber-200 dark:hover:bg-amber-900/50 text-amber-800 dark:text-amber-400 px-4 py-2 rounded-lg text-sm font-medium transition-colors">
        <CreditCardIcon class="w-4 h-4" />
        Expenses
      </Link>
      <Link :href="route('hive.finance.reports.income')" class="inline-flex items-center gap-2 bg-amber-100 dark:bg-amber-900/30 hover:bg-amber-200 dark:hover:bg-amber-900/50 text-amber-800 dark:text-amber-400 px-4 py-2 rounded-lg text-sm font-medium transition-colors">
        <ArrowDownIcon class="w-4 h-4" />
        Income Report
      </Link>
      <Link :href="route('hive.finance.reports.expenses')" class="inline-flex items-center gap-2 bg-amber-100 dark:bg-amber-900/30 hover:bg-amber-200 dark:hover:bg-amber-900/50 text-amber-800 dark:text-amber-400 px-4 py-2 rounded-lg text-sm font-medium transition-colors">
        <ArrowUpIcon class="w-4 h-4" />
        Expense Report
      </Link>
      <Link :href="route('hive.finance.reports.ageAnalysis')" class="inline-flex items-center gap-2 bg-amber-100 dark:bg-amber-900/30 hover:bg-amber-200 dark:hover:bg-amber-900/50 text-amber-800 dark:text-amber-400 px-4 py-2 rounded-lg text-sm font-medium transition-colors">

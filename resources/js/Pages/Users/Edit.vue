<template>
  <AppLayout :title="`Edit: ${user.name}`" description="Update user account and profile">
    <template #header-actions>
      <Link :href="route('users.show', user.id)" class="text-sm text-gray-500 hover:text-gray-700 font-medium">
        ← Back to Profile
      </Link>
    </template>
    ...
    <div v-if="confirmDelete" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
        ...
        <p class="text-sm text-gray-500 mb-6">
          This will permanently delete <strong>{{ user.name }}</strong>'s account and all associated profile data.
        </p>
...
const staffRoles = ['super-admin', 'school-admin', 'department-head', 'chef-instructor']
const sp = props.user.staff_profile
const stp = props.user.student_profile

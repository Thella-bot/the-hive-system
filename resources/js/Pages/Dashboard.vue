<template>
  <HiveLayout title="Dashboard" :description="currentYear ? `Academic Year: ${currentYear.name}` : 'No active academic year set'">
    ...
    <div class="mb-8 animate-fade-in">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-2 p-6 rounded-xl bg-gradient-to-r from-amber-50 to-white dark:from-amber-900/30 dark:to-gray-800 border border-amber-100 dark:border-amber-700 shadow-sm animate-fade-in">
        <div>
          <h2 class="text-2xl font-bold text-amber-700 dark:text-amber-300 mb-1">Welcome back, {{ $page.props.auth.user.name }}!</h2>
          <p class="text-gray-600 dark:text-gray-400">Here's a quick look at your dashboard for today.</p>
        </div>
        <div class="flex items-center gap-3 mt-4 md:mt-0">
          <img :src="$page.props.auth.user.profile_photo_url" :alt="$page.props.auth.user.name" class="w-14 h-14 rounded-full object-cover border-2 border-amber-300 dark:border-amber-600 shadow" />
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 mb-8 animate-fade-in-slow">
      ...
      <StatCard label="Staff" :value="stats.staff" sub="Academic & non-academic" icon-bg="bg-purple-100 dark:bg-purple-900/40">
        <template #icon>

<template>
  <IntranetLayout>
    <div class="flex justify-between mb-4">
      ...
      <Link v-if="canUpload" :href="route('intranet.payslips.create')" class="bg-indigo-600 text-white px-4 py-2 rounded">Upload</Link>
    </div>
    <table class="w-full bg-white shadow rounded">
      ...
      <tbody>
        <tr v-for="slip in payslips" :key="slip.id">
          ...
          <td>{{ slip.month }}</td>
          <td><a :href="route('intranet.payslips.download', slip.id)" class="text-blue-600 underline">Download</a></td>
        </tr>
      </tbody>
    </table>
  </IntranetLayout>

...
const navigation = computed(() => {
  ...
  if (isAcademicStaff.value || isAdmin.value) {
    items.push(
      {
        name: 'Teaching',
        icon: BookOpenIcon,
  ...
  if (userRoles.value.includes('super-admin')) {
    items.push({
      name: 'System',
  ...
  return items
    .map((item) => item.children ? { ...item, children: item.children.filter((child) => canAccess(child.roles)) } : item)
...
const resourceLinks = () => {
  ...
  // Non-academic staff get Events & Announcements only under Operations
  if (!isNonAcademicStaff.value) {
    links.push(
  ...
  if (isStaff.value || isAdmin.value) {
    ...
    links.push({ name: 'Documents', href: route('hive.documents.module-select'), active: 'hive.documents.*', roles: ['student'] });
  }
